...
var getWxLoginResult = function getLoginCode(callback) {
    wx.login({//获取code
        success: function (loginResult) {
            wx.getUserInfo({//获取用户信息，不包括openid
...
var login = function login(options) {
            ...
            success: function (result) {
               // console.log(result);
                ...
                if (data && data[constants.WX_SESSION_MAGIC_ID]) {
                    if (data.session) { //获得服务器返回的信息
                        data.session.userInfo = userInfo;//把用户信息放到session里
                        Session.set(data.session);//把服务器返回的信息放到缓存里
    ...
    if (session) {//检查缓存里是不是有用户信息
        wx.checkSession({//检查登录是否过期
            success: function () {
